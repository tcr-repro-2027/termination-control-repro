...
- 每 response 经历 episodes（capture/stop 前）：M0 中位 2.0（p25–p75 0.0–5.0），M1 中位 2.0（1.0–6.0）；mean Δ=0.57 [0.44, 0.69]。
- 0-episode（从无非空复用）：M0=26.7%，M1=24.2%，Δ=-2.6%[-3.7%,-1.3%]。
- 终局分布：capture M0/M1=4.4%/9.5%；normal-stop=95.6%/90.4%；hit-max 删失=0.0%/0.1%。
- captured 子集：capture episode ordinal 中位 M0=4.0，M1=5.0；capture 前已恢复 episodes mean M0=3.91，M1=5.03，Δ=1.12 [0.57, 1.64]；第 1 个 episode 即被 capture 的占比 M0=21.6%，M1=10.6%，Δ=-11.0%[-15.4%,-6.4%]。
- episode 长度（blocks）中位：M0=1.0，M1=1.0；被 capture 的 episode 长度中位：M0=21.0，M1=28.0。
- capture 后仍出现 novel block 的 captured responses（capture 逃逸迹象）：M0=14，M1=19。

## 4. Per-episode capture hazard h(k)=P(capture|第k个episode存在)（lineage，primary）

| k | h_M0(k) | h_M1(k) | Δ [95% CI] | 方向 |
| --- | --- | --- | --- | --- |
| 1 | 1.3% | 1.3% | 0.0%[-0.3%,0.4%] | UNRESOLVED |
| 2 | 1.1% | 2.0% | 0.9%[0.4%,1.4%] | UP |
| 3 | 1.1% | 2.9% | 1.8%[1.2%,2.4%] | UP |
| 4 | 1.6% | 2.3% | 0.8%[0.1%,1.4%] | UP |
| 5 | 1.4% | 2.9% | 1.4%[0.6%,2.2%] | UP |
| 6 | 1.6% | 3.9% | 2.3%[1.4%,3.3%] | UP |
| 7 | 1.2% | 2.8% | 1.7%[0.8%,2.5%] | UP |
| 8 | 1.9% | 3.1% | 1.2%[0.0%,2.3%] | UP |
| 9+ (pooled) | 1.0% | 2.2% | 1.2%[0.7%,1.7%] | UP |
| all (pooled) | 1.3% | 2.3% | 1.1%[0.9%,1.3%] | UP |

- gap stop hazard（每个恢复间隙的正常停止概率）pooled：M0=16.0%，M1=13.2%，Δ=-2.7%[-3.3%,-2.2%]。
- in-episode stop hazard（episode 内正常结束）pooled：M0=8.4%，M1=7.1%，Δ=-1.4%[-1.7%,-1.0%]。

## 5. Episode-ordinal CIF 与 Shapley 分解（主判定）

- capture CIF@K124：M0=4.4%，M1=9.5%，Δ=5.1%[4.2%,5.9%]。
- stop CIF@K124：M0=95.6%，M1=90.5%，Δ=-5.1%[-5.9%,-4.2%]。
- Shapley（尾部池化边界 K_pool=24，双模型 episode at-risk ≥50 的最大 ordinal；边界外每模型每 cause 用其自身尾部池化 hazard，避免空风险集填 0 伪造 propensity）：per-episode propensity 分量=3.7%[2.9%,4.4%]（share=71.9%）；stop/exposure 分量=1.4%[1.2%,1.7%]（share=28.1%）。
- 分解模型总差=5.1%[4.2%,5.9%]；原始曲线 ΔCIF=5.1%[4.2%,5.9%]（两者之差为尾部平滑差异，应当很小）；identity error=0.000000。

## 6. Legacy 口径一致性（lineage，legacy-aligned capture）

- capture 率 M0/M1=0.5%/1.3%；pooled per-episode hazard Δ=0.2%[0.1%,0.2%]；propensity 分量=0.7%[0.3%,1.0%]；exposure 分量=0.1%[0.1%,0.2%]。
- stable orbit 存在但结构未对齐（记删失）：M0=0，M1=2。

## 7. Orbit-stage flag（独立于 A/B）

- P(stable orbit | primary captured)：M0=11.6%，M1=13.6%，Δ=2.0% [-2.1%, 6.3%]；flag=OFF。
- P(stable orbit | uncaptured)：M0=0.0%，M1=0.0%。

## 8. 距离统计（captured responses，lineage，primary；描述性，无 CI）

- last response 内新 triple → capture episode 起点：M0 中位 1.0 blocks（p25–p75 1.0–4.0），M1 中位 1.0（1.0–5.0）。
- capture episode 是 last 新 triple 后第一个 episode 的占比：M0=58.9%，M1=50.2%；之间隔的 episodes 中位：M0=0.0，M1=0.0。
- last response 内新 pair → capture episode 起点中位：M0=2.0，M1=3.0（来源=reparse）。

## 9. 敏感性（contiguous 变体，primary 口径）

- pooled per-episode hazard Δ=1.8%[1.5%,2.2%]；propensity 分量=3.8%[3.1%,4.6%]；exposure 分量=1.3%[1.0%,1.5%]。
- 分支：lineage=B_PROPENSITY_ALSO_ELEVATED，contiguous=B_PROPENSITY_ALSO_ELEVATED。

## 10. 判定

- `P0D_B_PROPENSITY_ALSO_ELEVATED`
- 下一步：两阶段机制：终止失准+捕获增强；R1/S1 照常，R2 后追加 episode 起点固定前缀 F_core clamp 干预式验证（须过随机匹配对照）。
