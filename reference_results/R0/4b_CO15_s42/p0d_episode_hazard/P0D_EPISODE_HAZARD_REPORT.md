...
- 每 response 经历 episodes（capture/stop 前）：M0 中位 2.0（p25–p75 0.0–5.0），M1 中位 2.0（1.0–5.0）；mean Δ=0.24 [0.12, 0.37]。
- 0-episode（从无非空复用）：M0=27.5%，M1=23.5%，Δ=-4.0%[-5.2%,-2.8%]。
- 终局分布：capture M0/M1=5.5%/9.7%；normal-stop=94.4%/90.2%；hit-max 删失=0.1%/0.1%。
- captured 子集：capture episode ordinal 中位 M0=4.0，M1=4.0；capture 前已恢复 episodes mean M0=4.87，M1=4.81，Δ=-0.06 [-0.57, 0.45]；第 1 个 episode 即被 capture 的占比 M0=16.2%，M1=11.3%，Δ=-4.9%[-8.9%,-0.9%]。
- episode 长度（blocks）中位：M0=1.0，M1=1.0；被 capture 的 episode 长度中位：M0=21.0，M1=24.0。
- capture 后仍出现 novel block 的 captured responses（capture 逃逸迹象）：M0=15，M1=29。

## 4. Per-episode capture hazard h(k)=P(capture|第k个episode存在)（lineage，primary）

| k | h_M0(k) | h_M1(k) | Δ [95% CI] | 方向 |
| --- | --- | --- | --- | --- |
| 1 | 1.2% | 1.4% | 0.2%[-0.2%,0.6%] | UNRESOLVED |
| 2 | 1.1% | 2.3% | 1.1%[0.6%,1.6%] | UP |
| 3 | 1.7% | 2.8% | 1.1%[0.4%,1.8%] | UP |
| 4 | 1.5% | 3.0% | 1.5%[0.8%,2.3%] | UP |
| 5 | 1.6% | 2.8% | 1.2%[0.4%,2.0%] | UP |
| 6 | 1.6% | 2.5% | 0.8%[0.0%,1.6%] | UP |
| 7 | 2.0% | 3.6% | 1.6%[0.6%,2.7%] | UP |
| 8 | 1.5% | 2.8% | 1.2%[0.3%,2.3%] | UP |
| 9+ (pooled) | 1.7% | 2.7% | 1.0%[0.5%,1.5%] | UP |
| all (pooled) | 1.5% | 2.5% | 1.0%[0.7%,1.2%] | UP |

- gap stop hazard（每个恢复间隙的正常停止概率）pooled：M0=15.2%，M1=13.4%，Δ=-1.7%[-2.3%,-1.2%]。
- in-episode stop hazard（episode 内正常结束）pooled：M0=7.7%，M1=7.4%，Δ=-0.3%[-0.6%,0.1%]。

## 5. Episode-ordinal CIF 与 Shapley 分解（主判定）

- capture CIF@K51：M0=5.5%，M1=9.7%，Δ=4.2%[3.3%,5.1%]。
- stop CIF@K51：M0=94.5%，M1=90.3%，Δ=-4.2%[-5.1%,-3.3%]。
- Shapley（尾部池化边界 K_pool=24，双模型 episode at-risk ≥50 的最大 ordinal；边界外每模型每 cause 用其自身尾部池化 hazard，避免空风险集填 0 伪造 propensity）：per-episode propensity 分量=3.3%[2.5%,4.1%]（share=79.5%）；stop/exposure 分量=0.9%[0.6%,1.1%]（share=20.5%）。
- 分解模型总差=4.2%[3.3%,5.1%]；原始曲线 ΔCIF=4.2%[3.3%,5.1%]（两者之差为尾部平滑差异，应当很小）；identity error=0.000000。

## 6. Legacy 口径一致性（lineage，legacy-aligned capture）

- capture 率 M0/M1=0.6%/1.3%；pooled per-episode hazard Δ=0.2%[0.1%,0.2%]；propensity 分量=0.7%[0.4%,1.0%]；exposure 分量=0.1%[0.1%,0.1%]。
- stable orbit 存在但结构未对齐（记删失）：M0=1，M1=1。

## 7. Orbit-stage flag（独立于 A/B）

- P(stable orbit | primary captured)：M0=10.7%，M1=13.9%，Δ=3.2% [-0.9%, 7.1%]；flag=OFF。
- P(stable orbit | uncaptured)：M0=0.0%，M1=0.0%。

## 8. 距离统计（captured responses，lineage，primary；描述性，无 CI）

- last response 内新 triple → capture episode 起点：M0 中位 1.0 blocks（p25–p75 1.0–5.0），M1 中位 1.0（1.0–5.0）。
- capture episode 是 last 新 triple 后第一个 episode 的占比：M0=51.4%，M1=52.9%；之间隔的 episodes 中位：M0=0.0，M1=0.0。
- last response 内新 pair → capture episode 起点中位：M0=3.0，M1=3.0（来源=reparse）。

## 9. 敏感性（contiguous 变体，primary 口径）

- pooled per-episode hazard Δ=1.6%[1.2%,1.9%]；propensity 分量=3.3%[2.5%,4.1%]；exposure 分量=0.9%[0.7%,1.1%]。
