...
- 每 response 经历 episodes（capture/stop 前）：M0 中位 0.0（p25–p75 0.0–1.0），M1 中位 2.0（0.0–5.0）；mean Δ=3.05 [2.85, 3.24]。
- 0-episode（从无非空复用）：M0=70.5%，M1=27.5%，Δ=-43.0%[-44.9%,-41.1%]。
- 终局分布：capture M0/M1=1.3%/5.5%；normal-stop=98.7%/94.4%；hit-max 删失=0.0%/0.1%。
- captured 子集：capture episode ordinal 中位 M0=2.0，M1=4.0；capture 前已恢复 episodes mean M0=1.10，M1=4.87，Δ=3.77 [3.16, 4.38]；第 1 个 episode 即被 capture 的占比 M0=48.2%，M1=16.2%，Δ=-32.1%[-41.6%,-22.6%]。
- episode 长度（blocks）中位：M0=1.0，M1=1.0；被 capture 的 episode 长度中位：M0=5.0，M1=21.0。
- capture 后仍出现 novel block 的 captured responses（capture 逃逸迹象）：M0=9，M1=15。

## 4. Per-episode capture hazard h(k)=P(capture|第k个episode存在)（lineage，primary）

| k | h_M0(k) | h_M1(k) | Δ [95% CI] | 方向 |
| --- | --- | --- | --- | --- |
| 1 | 2.1% | 1.2% | -0.9%[-1.6%,-0.2%] | DOWN |
| 2 | 2.0% | 1.1% | -0.9%[-1.9%,0.0%] | UNRESOLVED |
| 3 | 2.3% | 1.7% | -0.6%[-1.8%,0.5%] | UNRESOLVED |
| 4 | 1.9% | 1.5% | -0.4%[-1.8%,0.9%] | UNRESOLVED |
| 5 | 2.1% | 1.6% | -0.4%[-2.5%,1.3%] | UNRESOLVED |
| 6 | 0.8% | 1.6% | 0.9%[-1.1%,2.1%] | UNRESOLVED |
| 7 | 0.0% | 2.0% | 2.0%[1.3%,2.6%] | UP |
| 8 | 1.9% | 1.5% | -0.4%[-4.8%,2.0%] | UNRESOLVED |
| 9+ (pooled) | 0.6% | 1.7% | 1.0%[-0.8%,2.0%] | UNRESOLVED |
| all (pooled) | 2.0% | 1.5% | -0.5%[-1.0%,0.0%] | UNRESOLVED |

- gap stop hazard（每个恢复间隙的正常停止概率）pooled：M0=56.0%，M1=15.2%，Δ=-40.9%[-43.0%,-38.7%]。
- in-episode stop hazard（episode 内正常结束）pooled：M0=24.9%，M1=7.7%，Δ=-17.2%[-18.5%,-16.0%]。

## 5. Episode-ordinal CIF 与 Shapley 分解（主判定）

- capture CIF@K41：M0=1.3%，M1=5.5%，Δ=4.2%[3.4%,5.1%]。
- stop CIF@K41：M0=98.7%，M1=94.5%，Δ=-4.2%[-5.1%,-3.4%]。
- Shapley（尾部池化边界 K_pool=8，双模型 episode at-risk ≥50 的最大 ordinal；边界外每模型每 cause 用其自身尾部池化 hazard，避免空风险集填 0 伪造 propensity）：per-episode propensity 分量=-0.7%[-1.9%,0.4%]（share=-17.7%）；stop/exposure 分量=5.0%[4.1%,6.0%]（share=117.7%）。
- 分解模型总差=4.2%[3.4%,5.1%]；原始曲线 ΔCIF=4.2%[3.4%,5.1%]（两者之差为尾部平滑差异，应当很小）；identity error=0.000000。

## 6. Legacy 口径一致性（lineage，legacy-aligned capture）

- capture 率 M0/M1=0.0%/0.6%；pooled per-episode hazard Δ=0.1%[0.0%,0.2%]；propensity 分量=0.3%[0.1%,0.4%]；exposure 分量=0.3%[0.2%,0.4%]。
- stable orbit 存在但结构未对齐（记删失）：M0=1，M1=1。

## 7. Orbit-stage flag（独立于 A/B）

- P(stable orbit | primary captured)：M0=1.8%，M1=10.7%，Δ=8.9% [4.2%, 13.1%]；flag=ON。
- P(stable orbit | uncaptured)：M0=0.0%，M1=0.0%。

## 8. 距离统计（captured responses，lineage，primary；描述性，无 CI）

- last response 内新 triple → capture episode 起点：M0 中位 1.0 blocks（p25–p75 1.0–1.0），M1 中位 1.0（1.0–5.0）。
- capture episode 是 last 新 triple 后第一个 episode 的占比：M0=76.3%，M1=51.4%；之间隔的 episodes 中位：M0=0.0，M1=0.0。
- last response 内新 pair → capture episode 起点中位：M0=1.0，M1=3.0（来源=reparse）。

## 9. 敏感性（contiguous 变体，primary 口径）

- pooled per-episode hazard Δ=-0.3%[-1.1%,0.4%]；propensity 分量=-1.1%[-2.4%,0.3%]；exposure 分量=5.3%[4.1%,6.5%]。
