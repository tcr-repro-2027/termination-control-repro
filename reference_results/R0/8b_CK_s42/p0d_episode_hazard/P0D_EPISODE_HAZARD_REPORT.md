...
- 每 response 经历 episodes（capture/stop 前）：M0 中位 1.0（p25–p75 0.0–5.0），M1 中位 4.0（1.0–8.0）；mean Δ=2.45 [2.27, 2.62]。
- 0-episode（从无非空复用）：M0=33.3%，M1=15.4%，Δ=-17.9%[-19.3%,-16.6%]。
- 终局分布：capture M0/M1=7.1%/31.8%；normal-stop=92.8%/67.9%；hit-max 删失=0.0%/0.3%。
- captured 子集：capture episode ordinal 中位 M0=4.0，M1=5.0；capture 前已恢复 episodes mean M0=4.83，M1=6.37，Δ=1.54 [1.04, 2.02]；第 1 个 episode 即被 capture 的占比 M0=14.1%，M1=10.6%，Δ=-3.5%[-6.6%,-0.6%]。
- episode 长度（blocks）中位：M0=1.0，M1=1.0；被 capture 的 episode 长度中位：M0=50.0，M1=359.0。
- capture 后仍出现 novel block 的 captured responses（capture 逃逸迹象）：M0=12，M1=56。

## 4. Per-episode capture hazard h(k)=P(capture|第k个episode存在)（lineage，primary）

| k | h_M0(k) | h_M1(k) | Δ [95% CI] | 方向 |
| --- | --- | --- | --- | --- |
| 1 | 1.5% | 4.0% | 2.5%[1.9%,3.1%] | UP |
| 2 | 1.7% | 5.0% | 3.3%[2.6%,3.9%] | UP |
| 3 | 2.9% | 5.5% | 2.6%[1.7%,3.5%] | UP |
| 4 | 2.6% | 6.4% | 3.7%[2.7%,4.7%] | UP |
| 5 | 2.3% | 6.6% | 4.3%[3.2%,5.4%] | UP |
| 6 | 2.6% | 6.6% | 4.0%[2.9%,5.1%] | UP |
| 7 | 2.7% | 6.7% | 4.1%[2.9%,5.3%] | UP |
| 8 | 2.3% | 7.0% | 4.7%[3.3%,6.1%] | UP |
| 9+ (pooled) | 2.3% | 5.5% | 3.2%[2.6%,3.8%] | UP |
| all (pooled) | 2.2% | 5.6% | 3.4%[3.1%,3.7%] | UP |

- gap stop hazard（每个恢复间隙的正常停止概率）pooled：M0=16.9%，M1=7.0%，Δ=-10.0%[-10.8%,-9.2%]。
- in-episode stop hazard（episode 内正常结束）pooled：M0=8.1%，M1=4.4%，Δ=-3.7%[-4.0%,-3.3%]。

## 5. Episode-ordinal CIF 与 Shapley 分解（主判定）

- capture CIF@K87：M0=7.1%，M1=31.9%，Δ=24.8%[23.2%,26.4%]。
- stop CIF@K87：M0=92.9%，M1=68.1%，Δ=-24.8%[-26.4%,-23.2%]。
- Shapley（尾部池化边界 K_pool=24，双模型 episode at-risk ≥50 的最大 ordinal；边界外每模型每 cause 用其自身尾部池化 hazard，避免空风险集填 0 伪造 propensity）：per-episode propensity 分量=11.2%[10.0%,12.5%]（share=45.4%）；stop/exposure 分量=13.5%[12.5%,14.5%]（share=54.6%）。
- 分解模型总差=24.8%[23.2%,26.4%]；原始曲线 ΔCIF=24.8%[23.2%,26.4%]（两者之差为尾部平滑差异，应当很小）；identity error=0.000000。

## 6. Legacy 口径一致性（lineage，legacy-aligned capture）

- capture 率 M0/M1=0.5%/6.1%；pooled per-episode hazard Δ=0.4%[0.3%,0.4%]；propensity 分量=4.5%[4.0%,5.1%]；exposure 分量=2.0%[1.8%,2.3%]。
- stable orbit 存在但结构未对齐（记删失）：M0=0，M1=7。

## 7. Orbit-stage flag（独立于 A/B）

- P(stable orbit | primary captured)：M0=7.1%，M1=19.3%，Δ=12.1% [9.6%, 14.5%]；flag=ON。
- P(stable orbit | uncaptured)：M0=0.0%，M1=0.1%。

## 8. 距离统计（captured responses，lineage，primary；描述性，无 CI）

- last response 内新 triple → capture episode 起点：M0 中位 2.0 blocks（p25–p75 1.0–7.0），M1 中位 2.0（1.0–8.0）。
- capture episode 是 last 新 triple 后第一个 episode 的占比：M0=43.7%，M1=44.3%；之间隔的 episodes 中位：M0=1.0，M1=1.0。
- last response 内新 pair → capture episode 起点中位：M0=4.0，M1=4.0（来源=reparse）。

## 9. 敏感性（contiguous 变体，primary 口径）

- pooled per-episode hazard Δ=7.0%[6.4%,7.6%]；propensity 分量=13.7%[12.3%,15.0%]；exposure 分量=11.1%[10.2%,11.9%]。
