...
- 每 response 经历 episodes（capture/stop 前）：M0 中位 2.0（p25–p75 0.0–5.0），M1 中位 5.0（2.0–9.0）；mean Δ=2.96 [2.78, 3.16]。
- 0-episode（从无非空复用）：M0=27.5%，M1=9.7%，Δ=-17.8%[-19.1%,-16.5%]。
- 终局分布：capture M0/M1=5.5%/32.3%；normal-stop=94.4%/67.3%；hit-max 删失=0.1%/0.4%。
- captured 子集：capture episode ordinal 中位 M0=4.0，M1=6.0；capture 前已恢复 episodes mean M0=4.87，M1=6.43，Δ=1.56 [1.02, 2.10]；第 1 个 episode 即被 capture 的占比 M0=16.2%，M1=8.4%，Δ=-7.8%[-11.2%,-4.5%]。
- episode 长度（blocks）中位：M0=1.0，M1=1.0；被 capture 的 episode 长度中位：M0=21.0，M1=325.0。
- capture 后仍出现 novel block 的 captured responses（capture 逃逸迹象）：M0=15，M1=127。

## 4. Per-episode capture hazard h(k)=P(capture|第k个episode存在)（lineage，primary）

| k | h_M0(k) | h_M1(k) | Δ [95% CI] | 方向 |
| --- | --- | --- | --- | --- |
| 1 | 1.2% | 3.0% | 1.8%[1.3%,2.3%] | UP |
| 2 | 1.1% | 4.3% | 3.2%[2.6%,3.8%] | UP |
| 3 | 1.7% | 5.1% | 3.4%[2.7%,4.1%] | UP |
| 4 | 1.5% | 5.4% | 3.9%[3.1%,4.7%] | UP |
| 5 | 1.6% | 5.6% | 4.0%[3.1%,4.9%] | UP |
| 6 | 1.6% | 5.8% | 4.1%[3.2%,5.1%] | UP |
| 7 | 2.0% | 6.2% | 4.2%[3.2%,5.3%] | UP |
| 8 | 1.5% | 4.9% | 3.3%[2.3%,4.4%] | UP |
| 9+ (pooled) | 1.7% | 5.0% | 3.3%[2.8%,3.8%] | UP |
| all (pooled) | 1.5% | 4.9% | 3.4%[3.1%,3.6%] | UP |

- gap stop hazard（每个恢复间隙的正常停止概率）pooled：M0=15.2%，M1=5.5%，Δ=-9.6%[-10.4%,-8.9%]。
- in-episode stop hazard（episode 内正常结束）pooled：M0=7.7%，M1=4.2%，Δ=-3.5%[-3.8%,-3.2%]。

## 5. Episode-ordinal CIF 与 Shapley 分解（主判定）

- capture CIF@K63：M0=5.5%，M1=32.4%，Δ=26.9%[25.2%,28.6%]。
- stop CIF@K63：M0=94.5%，M1=67.6%，Δ=-26.9%[-28.6%,-25.2%]。
- Shapley（尾部池化边界 K_pool=24，双模型 episode at-risk ≥50 的最大 ordinal；边界外每模型每 cause 用其自身尾部池化 hazard，避免空风险集填 0 伪造 propensity）：per-episode propensity 分量=14.2%[12.8%,15.6%]（share=52.7%）；stop/exposure 分量=12.7%[11.8%,13.7%]（share=47.3%）。
- 分解模型总差=26.9%[25.2%,28.6%]；原始曲线 ΔCIF=26.9%[25.2%,28.6%]（两者之差为尾部平滑差异，应当很小）；identity error=-0.000000。

## 6. Legacy 口径一致性（lineage，legacy-aligned capture）

- capture 率 M0/M1=0.6%/7.0%；pooled per-episode hazard Δ=0.5%[0.4%,0.6%]；propensity 分量=4.9%[4.3%,5.5%]；exposure 分量=2.4%[2.1%,2.7%]。
- stable orbit 存在但结构未对齐（记删失）：M0=1，M1=1。

## 7. Orbit-stage flag（独立于 A/B）

- P(stable orbit | primary captured)：M0=10.7%，M1=21.8%，Δ=11.1% [7.5%, 14.5%]；flag=ON。
- P(stable orbit | uncaptured)：M0=0.0%，M1=0.0%。

## 8. 距离统计（captured responses，lineage，primary；描述性，无 CI）

- last response 内新 triple → capture episode 起点：M0 中位 1.0 blocks（p25–p75 1.0–5.0），M1 中位 1.0（1.0–6.0）。
- capture episode 是 last 新 triple 后第一个 episode 的占比：M0=51.4%，M1=50.3%；之间隔的 episodes 中位：M0=0.0，M1=0.0。
- last response 内新 pair → capture episode 起点中位：M0=3.0，M1=3.0（来源=reparse）。

## 9. 敏感性（contiguous 变体，primary 口径）

- pooled per-episode hazard Δ=6.5%[6.0%,7.0%]；propensity 分量=15.6%[14.2%,17.0%]；exposure 分量=11.3%[10.5%,12.1%]。
