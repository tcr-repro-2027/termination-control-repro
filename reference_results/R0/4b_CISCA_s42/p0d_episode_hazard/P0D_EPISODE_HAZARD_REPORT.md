...
- 每 response 经历 episodes（capture/stop 前）：M0 中位 2.0（p25–p75 0.0–5.0），M1 中位 3.0（1.0–6.0）；mean Δ=0.56 [0.43, 0.69]。
- 0-episode（从无非空复用）：M0=27.5%，M1=20.7%，Δ=-6.8%[-8.0%,-5.6%]。
- 终局分布：capture M0/M1=5.5%/3.9%；normal-stop=94.4%/96.1%；hit-max 删失=0.1%/0.0%。
- captured 子集：capture episode ordinal 中位 M0=4.0，M1=4.0；capture 前已恢复 episodes mean M0=4.87，M1=3.95，Δ=-0.92 [-1.52, -0.31]；第 1 个 episode 即被 capture 的占比 M0=16.2%，M1=18.4%，Δ=2.2%[-3.1%,7.6%]。
- episode 长度（blocks）中位：M0=1.0，M1=1.0；被 capture 的 episode 长度中位：M0=21.0，M1=19.0。
- capture 后仍出现 novel block 的 captured responses（capture 逃逸迹象）：M0=15，M1=9。

## 4. Per-episode capture hazard h(k)=P(capture|第k个episode存在)（lineage，primary）

| k | h_M0(k) | h_M1(k) | Δ [95% CI] | 方向 |
| --- | --- | --- | --- | --- |
| 1 | 1.2% | 0.9% | -0.3%[-0.7%,0.0%] | UNRESOLVED |
| 2 | 1.1% | 1.0% | -0.1%[-0.5%,0.2%] | UNRESOLVED |
| 3 | 1.7% | 1.0% | -0.7%[-1.2%,-0.2%] | DOWN |
| 4 | 1.5% | 1.2% | -0.3%[-0.9%,0.3%] | UNRESOLVED |
| 5 | 1.6% | 0.9% | -0.7%[-1.4%,-0.1%] | DOWN |
| 6 | 1.6% | 1.1% | -0.6%[-1.2%,0.1%] | UNRESOLVED |
| 7 | 2.0% | 1.3% | -0.7%[-1.5%,0.1%] | UNRESOLVED |
| 8 | 1.5% | 0.4% | -1.1%[-1.8%,-0.4%] | DOWN |
| 9+ (pooled) | 1.7% | 0.6% | -1.0%[-1.4%,-0.7%] | DOWN |
| all (pooled) | 1.5% | 0.9% | -0.6%[-0.8%,-0.4%] | DOWN |

- gap stop hazard（每个恢复间隙的正常停止概率）pooled：M0=15.2%，M1=12.8%，Δ=-2.4%[-2.9%,-1.8%]。
- in-episode stop hazard（episode 内正常结束）pooled：M0=7.7%，M1=7.9%，Δ=0.3%[-0.1%,0.6%]。

## 5. Episode-ordinal CIF 与 Shapley 分解（主判定）

- capture CIF@K68：M0=5.5%，M1=3.9%，Δ=-1.6%[-2.3%,-1.0%]。
- stop CIF@K68：M0=94.5%，M1=96.1%，Δ=1.6%[1.0%,2.3%]。
- Shapley（尾部池化边界 K_pool=24，双模型 episode at-risk ≥50 的最大 ordinal；边界外每模型每 cause 用其自身尾部池化 hazard，避免空风险集填 0 伪造 propensity）：per-episode propensity 分量=-2.2%[-2.8%,-1.5%]（share=132.1%）；stop/exposure 分量=0.5%[0.4%,0.7%]（share=-32.1%）。
- 分解模型总差=-1.6%[-2.3%,-1.0%]；原始曲线 ΔCIF=-1.6%[-2.3%,-1.0%]（两者之差为尾部平滑差异，应当很小）；identity error=0.000000。

## 6. Legacy 口径一致性（lineage，legacy-aligned capture）

- capture 率 M0/M1=0.6%/0.4%；pooled per-episode hazard Δ=-0.1%[-0.1%,-0.0%]；propensity 分量=-0.2%[-0.5%,-0.0%]；exposure 分量=0.1%[0.0%,0.1%]。
- stable orbit 存在但结构未对齐（记删失）：M0=1，M1=0。

## 7. Orbit-stage flag（独立于 A/B）

- P(stable orbit | primary captured)：M0=10.7%，M1=11.1%，Δ=0.4% [-3.8%, 4.7%]；flag=OFF。
...
- pooled per-episode hazard Δ=-1.0%[-1.3%,-0.7%]；propensity 分量=-2.3%[-3.0%,-1.7%]；exposure 分量=0.7%[0.6%,0.8%]。
- 分支：lineage=UNRESOLVED，contiguous=UNRESOLVED。

## 10. 判定

- `P0D_UNRESOLVED`
- 下一步：分解不可判定；先核对 lineage/contiguous 差异与 slot 级计数，再定分支。
