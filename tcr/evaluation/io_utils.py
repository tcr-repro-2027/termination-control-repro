...
class JsonlWriter:
    """Append-mode jsonl writer that keeps the handle open.

    `append_jsonl` reopens per record, which is right for a resumable file
    (1106 records) and wrong for a streamed one (8848 rows): the analysis stage
    writes to a `.partial` file it renames at the end, so per-line durability
    buys nothing and per-line `open()` costs real time.
    """
